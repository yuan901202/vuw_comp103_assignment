...
public class ExerciseUndo implements UIButtonListener, UIMouseListener{
    ...
    public void mousePerformed(String action, double x, double y) {
		...
		if (pointAtPlace(x, y, DISCARD1_X, DISCARD_Y)) {
		    ...
		    pile1.push(card);
		    // YOUR CODE HERE
		    undoStack.push(new UndoRecord(selected, pile1));
		    // END OF YOUR CODE
		    selected = -1;
		    redraw();
                }
		    ...
		    pile2.push(card);
		    // YOUR CODE HERE
		    undoStack.push(new UndoRecord(selected, pile2));
		    // END OF YOUR CODE
		    selected = -1;
		    redraw();
		}
	...
	if (!undoStack.isEmpty()){
	    UndoRecord undoR = undoStack.pop();
	    hand.add(undoR.getHandPosition(), undoR.getPile().pop());
	}
	// END OF YOUR CODE
    }
