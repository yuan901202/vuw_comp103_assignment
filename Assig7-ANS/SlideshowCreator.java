...
public class SlideshowCreator implements UIButtonListener
{
    ...
    private ImageNode reverse(ImageNode curr, ImageNode prev) {
        ImageNode newFirst = null;
        ImageNode next = curr.getNext();
        if (next != null) {
            newFirst = this.reverse(next, curr);
        } else {
            newFirst = curr;
        }
        curr.setNext(prev);
    ...
    private void addImage() {
        // YOUR CODE HERE
        String imageFileName = UIFileChooser.open("select an image");
        if ((this.images == null) || (this.selectedIndex == 0)) {
            this.images = new ImageNode(imageFileName, this.images);
        } else {
            this.images.add(imageFileName, this.selectedIndex);
        }
        this.redraw();
        // END OF YOUR CODE
    }
    ...
    private void moveSelectionRight() {
        // YOUR CODE HERE
        if (images != null) {
            int listSize = this.images.size();
            if ((this.selectedIndex + 1) < listSize) {
                this.selectedIndex++;
            }
        } 
        this.redraw();
    ...
    private void redraw() { 
        ...
        while (curr != null) {
            ...
            String imageFileName = curr.getName();
            UI.drawImage(imageFileName, x, y, currentnodewidth, currentnodewidth);
