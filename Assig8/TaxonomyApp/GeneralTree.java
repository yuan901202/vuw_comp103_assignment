...
public class GeneralTree{
    ...
    public void addNode(String data, String parent) {
        ...
        if(findNode(data) != null){
            UI.println("Cannot add " + data + " because it already exists!");
            return;
        }
    ...
    public void moveNode(String target, String destination) {
        ...
        GeneralTreeNode moveTo = this.findNode(destination);
        if((node == null) || (node == root) || (moveTo == null)){
            return;
        }
        if(inSubtree(moveTo, node)){
            return;
        }
        GeneralTreeNode parent = node.getParent();
        parent.getChildren().remove(node);
        moveTo.getChildren().add(node);
        node.setParent(moveTo);
    }
    ...
    public void listChain(String target) {
        ...
        UI.println("Chain: " + targetNode.getData());
        while(tempNode != root){
    ...
    public void listDepth(int depth) {
        // YOUR CODE HERE
        if(depth < 0){
            UI.println("Invalid depth! Try it again!");
            return;
        }
    ...
    public String findClosestCommonAncestor(String target1, String target2) {
        // YOUR CODE HERE
        GeneralTreeNode node1 = this.findNode(target1);
        GeneralTreeNode node2 = this.findNode(target2);
        if(node1 == null || node2 == null){
            return null;
        }
        
        GeneralTreeNode tempNode = node1;
        while(!inSubtree(node2, tempNode) && tempNode != root){
            tempNode = tempNode.getParent();
        }
