...
public class TwitterCloud implements UIButtonListener {
	...
	if (button.equals("Read File")){
	    UI.clearText();
	    UI.clearGraphics();
	    String fileName = UIFileChooser.open("Choose tweet file:");
	    if (fileName!= null){
		readTweetFile(fileName);
		UI.println("Read " + wordFrequency.size() + " different words");
	    }
	}
	if (button.equals("Draw Cloud")){
	    if (wordFrequency == null){UI.println("No file has been read yet");}
	    else {
		UI.clearGraphics();
		drawCloud();
	    }
	    ...
	    if (wordIndex == null){UI.println("No file has been read yet");}
	    else {
		UI.clearText();
...
*/
	...
	try{
		...
		while (sc.hasNext()){
			...
			int previous = wordFrequency.get(word);
			previous += 1;
			wordFrequency.put(word, previous);
			List<String> matchingTweets = wordIndex.get(word);
...
*/
	...
	for (String word : wordFrequency.keySet()) {
		...
		setSizeAndColor(freq);
		UI.drawString(word, x, y);
	    }
	}
	// END OF YOUR CODE
	/* Or, could use the hashCode of the word to get a repeatable place;
		int hash = word.hashCode();
		int x = cloudLeft + Math.abs(hash % cloudWidth);
		int y = cloudTop + Math.abs(hash / cloudWidth % cloudHeight);
	*/
    }
