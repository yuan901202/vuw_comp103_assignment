...
public class Sokoban implements UIButtonListener, UIKeyListener {
    ...
    private Stack<ActionRecord> records = new Stack<ActionRecord>(); 
    // Constructors
    ...
    if ( cells[newP.row][newP.col].hasBox() && cells[nextP.row][nextP.col].free() ) {
        push(dir);
        records.push(new ActionRecord("push", dir));
    }
    else if ( cells[newP.row][newP.col].free() ) {
        move(dir);
        records.push(new ActionRecord("move", dir));
    }
    }
    ...
    public void undo(){
        ...
        ActionRecord x = records.pop();
        if (x.isMove()){
            Trace.println("undo move " + x.dir());
            move(oppositeDirection(x.dir()));
        }
        else{
	       Trace.println("undo push " + x.dir());
	       pull(oppositeDirection(x.dir()));
	    }
    }
